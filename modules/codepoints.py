...
def u(caesar, input): 
   ...
   arg = input.bytes[3:]
   # caesar.msg('#inamidst', '%r' % arg)
   if not arg: 
      return caesar.reply('You gave me zero length input.')
   elif not arg.strip(' '): 
      if len(arg) > 1: return caesar.reply('%s SPACEs (U+0020)' % len(arg))
      return caesar.reply('1 SPACE (U+0020)')
   ...
   if printable: 
      ...
      if len(arg) == 4: 
         ...
         else: return caesar.say(about(u))
      ...
      if extended: 
         ...
         for i, result in enumerate(results): 
            if (i < 2) or ((i == 2) and (len(results) < 4)): 
               ...
               caesar.say(result + ' [...]')
         if not results: 
            caesar.reply('Sorry, no results')
      else: 
         # look up a codepoint freely
         result = codepoint_simple(arg)
         if result is not None: 
            caesar.say(result)
         else: caesar.reply("Sorry, no results for %r." % arg)
   else: 
      ...
      # look up more than three podecoints
         ...
         caesar.reply(' '.join('U+%04X' % ord(c) for c in text))
      else: caesar.reply('Sorry, your input is too long!')
u.commands = ['u']
u.example = '.u 203D'

def bytes(caesar, input): 
   """Show the input as pretty printed bytes."""
   b = input.bytes
   caesar.reply('%r' % b[b.find(' ') + 1:])
